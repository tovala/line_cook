...
def monthly_experian_password_update():
  ...
  def fetch_new_password():
      ...
      try:
        password_html = requests.post(
          url= 'https://ss3.experian.com/securecontrol/reset/passwordreset?command=requestnewpassword&application=netconnect&version=1',
          headers={
        ...
        password_html.raise_for_status()
        token_response_json = password_html.json()
      ...
      password_html = cap.send_request("POST", new_pw_url, data={}, headers=headers, auth=authorization)
      if not password_html:
          # Indicates that the post has failed
          return None
      new_password = re.findall("<Response><newPassword>(.*)</newPassword></Response>", password_html)
      if len(new_password) == 1:
          return new_password[0] 
      # Indicates that the password returned was malformed
      return None
