WITH selection_counts AS (
  SELECT
    ...
    , SUM(msa.meal_selection_count) AS menu_meal_selection_count
    , SUM(msa.selection_total_count) AS subterm_meal_selection_count
...
SELECT
  ...
  , facility_network
  , cycle
  , menu_meal_selection_count / subterm_meal_selection_count AS menu_raw_engagement_rate 
  ...
  , SUM(menu_meal_selection_count) OVER (PARTITION BY term_id, internal_meal_id, cycle) 
    ...
    AS cycle_raw_engagement_rate
  -- Engagement rate of a meal in cycle level(sum across facilities, in a specific cycle, and term)
  ...
  , SUM(menu_meal_selection_count) OVER (PARTITION BY term_id, internal_meal_id, facility_network) 
    ...
    AS facility_raw_engagement_rate
  -- Engagement rate of a meal in facility level(sum across cycles, in a specific facility, and term)
  -- Using to compareing engagement between facility
