    ...
    {% for bucket in price_buckets %}
      ...
      , aggs_{{ counter.value }} AS (
        SELECT
          ...
          , SUM(f.cohort_count) AS cohort_order_count
          , SUM(f.fulfilled_orders) AS order_count
      ...
      , retention_{{ counter.value }} as (
        SELECT
          adjusted_order_week
          , ZEROIFNULL(DIV0NULL(order_count, cohort_order_count)) AS order_retention_{{ start_month }}_{{ commitment }}_{{ bucket[0] | replace('-', '_') }}
