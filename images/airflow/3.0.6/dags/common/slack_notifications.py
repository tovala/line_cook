...
def slack_param(channel_name: str='#team-data-notifications'):
  ...
  '''
  return Param(channel_name, type=['string', 'null'], description='Slack channel to send alerts. Default #team-data-notifications. If a private channel, the Airflow Notifications slack app MUST be explicitly invited into the channel.')
...
def dbt_failure_callback(context: Dict[str, Any]):
  '''
  '''
